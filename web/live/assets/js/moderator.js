(function () {
  ...
  function esc(v) { return ui?.esc ? ui.esc(v) : String(v ?? ''); }
  function fmt(v) { return ui?.fmtDate ? ui.fmtDate(v) : (v || '—'); }
  function flash(msg, type = 'info') { if (ui?.flash) ui.flash(msg, type); }
  function setConnection(ok, label) { if (ui?.setConnection) ui.setConnection(ok, label); }
  ...
  async function refreshAll() {
    const [roomsRes, membershipsRes, schedulesRes, pulseRes, promptsRes, showRes, msgRes] = await Promise.all([
      ...
      db.client.from('patron_pulse').select('*').order('created_at', { ascending: false }).limit(100),
      db.client.from('pulse_prompts').select('*').order('created_at', { ascending: false }).limit(100),
      db.client.from('show_state').select('*'),
      db.client.from('production_messages').select('*').order('created_at', { ascending: false }).limit(100),
    ]);

    [roomsRes, membershipsRes, schedulesRes, pulseRes, promptsRes, showRes, msgRes].forEach(r => { if (r.error) throw r.error; });
  ...
  function renderRoomAdminTable() {
    ...
    tbody.querySelectorAll('[data-edit-room]').forEach(btn => btn.onclick = () => {
    ...
    tbody.querySelectorAll('[data-manage-room]').forEach(btn => btn.onclick = () => {
    ...
    tbody.querySelectorAll('[data-delete-room]').forEach(btn => btn.onclick = async () => {
  ...
  function renderManageRoomSelect() {
    ...
    select.innerHTML = (ui.option ? ui.option('', 'Select room') : '<option value="">Select room</option>') + state.rooms.map(r => ui.option ? ui.option(r.id, r.title, r.id === current) : `<option value="${esc(r.id)}">${esc(r.title)}</option>`).join('');
  ...
  function renderManageRoomPanel(roomId) {
    ...
    const avgPulse = roomPulseEntries.length ? Math.round(roomPulseEntries.reduce((a, b) => a + Number(b.pulse_score || 0), 0) / roomPulseEntries.length) : 0;
    ...
    if (summary) {
      summary.innerHTML = room ? `
        ...
        <div class="stat-box"><strong>${esc(show?.current_segment || '—')}</strong><span>Current Segment</span></div>
    ...
    if (venuesBody) {
      venuesBody.innerHTML = members.map(m => `
        <tr>
          <td>${esc(m.venue_id)}</td>
          <td>${esc(m.status || 'connected')}</td>
          <td>${m.is_broadcasting ? 'Live' : 'Idle'}</td>
          <td class="actions">
            <button type="button" data-toggle-feed="${m.id}">${m.is_broadcasting ? 'Stop Feed' : 'Start Feed'}</button>
            <button type="button" class="danger" data-kick-venue="${m.id}">Disconnect</button>
          </td>
        </tr>`).join('') || '<tr><td colspan="4">No connected venues.</td></tr>';

      venuesBody.querySelectorAll('[data-toggle-feed]').forEach(btn => btn.onclick = async () => {
        try {
          const member = members.find(m => m.id === btn.dataset.toggleFeed);
          const { error } = await db.client.from('room_venues').update({
      ...
      venuesBody.querySelectorAll('[data-kick-venue]').forEach(btn => btn.onclick = async () => {
        if (!confirm('Disconnect this venue from the room?')) return;
        try {
          const { error } = await db.client.from('room_venues').update({ left_at: new Date().toISOString(), status: 'removed', is_broadcasting: false }).eq('id', btn.dataset.kickVenue);
          if (error) throw error;
    ...
    if (pulseBox) {
      pulseBox.innerHTML = prompt
        ? `<h4>${esc(prompt.prompt_text)}</h4><p>Type: ${esc(prompt.prompt_type || 'vote')}</p><p>Ends: ${fmt(prompt.ends_at)}</p><p>Entries: ${roomPulseEntries.length}</p>`
        : '<p>No live pulse prompt for this room.</p>';
    }

    if (msgBody) {
      msgBody.innerHTML = roomMessages(roomId).map(m => `<tr><td>${esc(m.from_role)}</td><td>${esc(m.body)}</td><td>${fmt(m.created_at)}</td></tr>`).join('') || '<tr><td colspan="3">No room messages.</td></tr>';
    }
  ...
  function bindShowStateForm() {
    ...
    document.getElementById('production-override-stop')?.addEventListener('click', async () => {
      try {
        ...
        const { error } = await db.client.from('show_state').upsert({ room_id: roomId, portal_open: false, timer_running: false, event_type: 'stopped', updated_at: new Date().toISOString() }, { onConflict: 'room_id' });
        if (error) throw error;
        flash('Production override applied.');
        await refreshAll();
      } catch (err) {
  ...
  function renderScheduleTable() {
    ...
    tbody.innerHTML = state.schedules.map(s => `
      ...
      </tr>`).join('') || '<tr><td colspan="7">No schedule segments found.</td></tr>';

    tbody.querySelectorAll('[data-edit-schedule]').forEach(btn => btn.onclick = () => {
      ...
      document.getElementById('schedule-form-title').textContent = 'Edit Schedule Segment';
    });

    tbody.querySelectorAll('[data-delete-schedule]').forEach(btn => btn.onclick = async () => {
  ...
  function bindScheduleForm() {
    ...
    if (roomSelect) {
      roomSelect.innerHTML = (ui.option ? ui.option('', 'Select room') : '<option value="">Select room</option>') + state.rooms.map(r => ui.option ? ui.option(r.id, `${r.title} (${r.zone || 'Local'})`) : `<option value="${esc(r.id)}">${esc(r.title)}</option>`).join('');
    }
    ...
    form.addEventListener('submit', async e => {
      ...
      try {
        const payload = {
          ...
          venue_id: state.rooms.find(r => r.id === form.room_id.value)?.venue_id || null,
        };
        if (form.dataset.editingId) {
        ...
        form.reset();
        form.dataset.editingId = '';
        document.getElementById('schedule-form-title').textContent = 'Create Schedule Segment';
        await refreshAll();
      } catch (err) {
    ...
    document.querySelectorAll('[data-quick-segment]').forEach(btn => btn.addEventListener('click', () => {
  ...
  async function boot() {
    try {
      ...
      ['rooms', 'room_venues', 'schedules', 'patron_pulse', 'pulse_prompts', 'show_state', 'production_messages'].forEach(t => db.subscribe(t, refreshAll));
    } catch (err) {
      console.error(err);
